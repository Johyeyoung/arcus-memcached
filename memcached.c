...
static void complete_update_ascii(conn *c)
{
    ...
    if (ret != ENGINE_SUCCESS ||
        ...
        /* In case of append and prepend operations,
         * a new item is allocated and stored in the storage engine.
         * So, free the item that has only append/prepend value.
         */
        mc_engine.v1->free(mc_engine.v0, c, c->item);
    }
    c->store_op = 0;
...
static void process_mop_command(conn *c, token_t *tokens, const size_t ntokens)
{
    ...
    else if ((ntokens >= 6 && ntokens <= 8) && (strcmp(subcommand, "delete") == 0))
    {
        ...
        if (numfields == 0) {
            ...
            if (numfields > settings.max_map_size ||
